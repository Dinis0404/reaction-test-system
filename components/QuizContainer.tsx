...
<<<<<<< HEAD
  ...
  const submitAllAnswers = async () => {
    console.log('submitAllAnswers 被調用');
    console.log('當前狀態:', { isPracticeMode, questionsCount: questions.length, answersCount: Object.keys(answers).length, hasSessionData: !!sessionData });
    ...
    if (isPracticeMode) {
      ...
      try {
        ...
        console.log('練習模式結果計算完成');
      } catch (err) {
        console.error('練習模式結果計算錯誤:', err);
        alert('計算結果時發生錯誤: ' + (err instanceof Error ? err.message : '未知錯誤'));
      } finally {
    ...
    console.log('檢查未回答題目:', { answeredCount, totalQuestions });
...
<<<<<<< HEAD
      ...
      // 驗證必要數據
      if (!sessionData) {
        throw new Error('缺少會話數據');
      }
      
      if (!questions || questions.length === 0) {
        throw new Error('沒有題目數據');
      }
      ...
      console.log('準備提交的答案:', answerArray);
      console.log('會話數據樣本:', JSON.stringify(sessionData, null, 2));

      // 提交答案到服務器
      console.log('發送請求到 /api/quiz/complete');
...
<<<<<<< HEAD
      ...
      console.log('收到服務器響應:', { status: answerResponse.status, ok: answerResponse.ok });

      if (!answerResponse.ok) {
        const errorText = await answerResponse.text();
        console.error('服務器錯誤響應:', errorText);
        let errorMessage = '提交答案失敗';
        try {
          const errorData = JSON.parse(errorText);
          errorMessage = errorData.error || errorData.message || errorMessage;
        } catch {
          errorMessage = `服務器錯誤 (${answerResponse.status})`;
        }
        throw new Error(errorMessage);
      }

      // 獲取結果
      const resultData = await answerResponse.json();
      console.log('收到結果數據:', resultData);
      
      // 設置結果並顯示
      setResults(resultData.results);
      setOriginalResults(resultData.results);
      
      // 更新會話數據
      if (resultData.sessionData) {
        setSessionData(resultData.sessionData);
      }
      try {
        const summary = {
          score: resultData.summary?.score ?? (Math.round((resultData.results.filter((r: any) => r.isCorrect).length / resultData.results.length) * 100)),
          correctCount: resultData.summary?.correctCount ?? resultData.results.filter((r: any) => r.isCorrect).length,
          totalQuestions: resultData.summary?.totalQuestions ?? resultData.results.length,
        };
        const ctx = {
          sessionId: resultData.sessionData?.startTime,
          selectedFiles,
          folder: selectedFolder,
          userId: getActiveUserId() || undefined,
        };
        saveResult(summary, resultData.results.map((r: any) => ({
        ...
        console.log('保存成績失敗', e);
      }
      
      console.log('答案提交成功，結果已獲取');
    } catch (err) {
      console.error('提交答案錯誤:', err);
      if (err instanceof Error && err.name === 'AbortError') {
        alert('請求超時，請檢查網絡連接或稍後再試');
      } else {
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    setAiAnalysis(null);
                    const summary = { score, correctCount, totalQuestions };
                    ...
                    setAnalysisError(e?.message || '分析失敗');
                  } finally {
                    setIsAnalyzing(false);
                  }
                }}
                className="px-8 py-4 bg-gradient-to-r from-indigo-500 to-purple-600 text-white rounded-xl hover:from-indigo-600 hover:to-purple-700 transition-all font-bold text-lg shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-900 min-h-[56px] flex items-center justify-center gap-2"
              >
...
<<<<<<< HEAD
        {aiAnalysis && (
          <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl shadow-xl p-6 border border-gray-700 mb-6">
            ...
            <div className="text-gray-200 whitespace-pre-wrap leading-relaxed">{aiAnalysis}</div>
          </div>
        )}
        {analysisError && (
...
<<<<<<< HEAD
                onClick={() => {
                  ...
                  console.log('答案數量:', Object.keys(answers).length);
